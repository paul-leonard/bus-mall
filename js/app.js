...
function renderAllTimeGraphs() {
  ...
  for (var i = 0; i < ProductObject.arrayOfProductObjects.length; i++) {
    productNameForGraph.push(ProductObject.arrayOfProductObjects[i].productName);
    voteDataForGraph.push(ProductObject.arrayOfProductObjects[i].chosenAllTimeCount);
    shownDataForGraph.push(ProductObject.arrayOfProductObjects[i].shownAllTimeCount);
  }

  var ctx4 = document.getElementById('allTimeGraph').getContext('2d');

  var votesBarGraph = new Chart(ctx4, {
    type: 'bar',
    data: {
      labels: productNameForGraph,
      datasets: [{
